...
import { AppDataContextType } from "hooks/pool/list/useUpdatePoolSecondaryData";
...
const useTokenRewardData = (
  ...
  const fetchTokenRewardData = useCallback(async () => {
    ...
    try {
      ...
      const { baseAssetData, priceFeedData, totalPoolData } = appData;
      if (!baseAssetData || !priceFeedData || !totalPoolData) {
        setTokenRewardData(undefined);
        return;
      }
      const totalBaseSupply = totalPoolData[symbol]?.totalBaseSupplyBalance ?? 0;
      const totalBaseBorrow = totalPoolData[symbol]?.totalBaseBorrowBalance ?? 0;
      ...
      const baseAssetPrice = priceFeedData[symbol]?.baseAsset ?? 0;
      const rewardAssetPrice = priceFeedData[symbol]?.rewardAsset ?? 0;
